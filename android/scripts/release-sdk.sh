...
echo "Releasing Meet.NUG SDK ${SDK_VERSION}"
...
# Now build and publish the Meet.NUG SDK and its dependencies
echo "Building and publishing the Meet.NUG SDK"
pushd ${THIS_DIR}/../
./gradlew clean
...
if [[ $MVN_HTTP == 0 ]]; then
    pushd ${MVN_REPO_PATH}
    git add -A .
    git commit -m "Meet.NUG SDK + dependencies: ${SDK_VERSION}"
    popd
fi
